...
def _vendor_name_from_from_header(from_header: str) -> str:
    match = re.match('^\\s*\\"?([^\\"<]+?)\\"?\\s*<', from_header)
    if match:
        return match.group(1).strip()
    return from_header.strip() or 'Unknown sender'

def _email_from_from_header(from_header: str) -> str:
    match = re.search('<([^>]+)>', from_header)
    if match:
...
def run_once(gmail: GmailClient, extractor: AttachmentExtractor, cfg: dict, store: ProcessedIdStore):
    ...
    for msg_ref in messages:
        ...
        gmail.modify_labels(message_id, add=[processed_label_id], remove=['UNREAD'])
        store.add(message_id)
        new_count += 1
        message_records.append({'message_id': message_id, 'subject': _header_value(full_message, 'Subject') or '(no subject)', 'vendor_name_raw': _vendor_name_from_from_header(_header_value(full_message, 'From')), 'sender_email': _email_from_from_header(_header_value(full_message, 'From')), 'received_date': _received_date(full_message), 'saved_files': saved_files or []})
        if saved_files:
