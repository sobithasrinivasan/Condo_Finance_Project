...
export interface SelectLedgerModelProps {
    isOpen: boolean;
    onClose: () => void;
    onConfirmMatch?: (ledgerRecord: LedgerRecord) => void;
    bankTransaction?: {
        description: string;
        amount: string;
        date: string;
...
export default function SelectLedgerModel({
    ...
    const selectedLedger = mockLedgers.find(l => l.id === selectedLedgerId) || mockLedgers[0];
                                    ...
                                    >
                                        <td className="py-3.5 px-3 text-center">
                                            <div className={`w-4 h-4 rounded-full border flex items-center justify-center mx-auto transition-colors ${
                                                isSelected ? "border-[#0B46AD] bg-[#0B46AD]" : "border-slate-300"
                                            }`}>
