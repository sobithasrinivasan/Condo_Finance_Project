...
export interface ManualPaymentDetailsType {
    invoiceNo?: string;
    vendor?: string;
    amountPaid?: string;
    paymentDate?: string;
...
interface ManualMatchSuccessProps {
    isOpen?: boolean;
    onClose?: () => void;
    details?: ManualPaymentDetailsType | null;
}

export default function ManualMatchSuccess({
    isOpen = true,
    onClose,
    details,
}: ManualMatchSuccessProps) {
    if (!isOpen) return null;

    const payment: ManualPaymentDetailsType = {
        invoiceNo: details?.invoiceNo || "INV-1045",
        vendor: details?.vendor || "Amazon Subscription",
        amountPaid: details?.amountPaid || "$89.99",
        paymentDate: details?.paymentDate || "Jul 07, 2026",
        paymentMethod: details?.paymentMethod || "Bank Statement",
        referenceNo: details?.referenceNo || "AMZ-3345",
    };
                ...
                <div className="space-y-3 pt-2">
                    ...
                    <div className="space-y-1">
                        ...
                        <p className="text-xs sm:text-sm text-slate-500 max-w-xs mx-auto leading-relaxed">
                            The transaction has been successfully matched and the invoice is marked as Paid.
                        </p>
                    </div>
                </div>
