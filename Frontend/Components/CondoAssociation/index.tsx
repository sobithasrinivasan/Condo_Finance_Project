...
import {
    getCondoAssociationApi,
    createCondoAssociationApi,
    updateCondoAssociationApi,
    deleteCondoAssociationApi,
    CondoAssociationType
} from "@/api/CondoAssociation/condoAssociationApi";
...
export default function CondoAssociation() {
    ...
    const fetchAssociations = async () => {
        try {
            setLoading(true);
            const response = await getCondoAssociationApi();
            if (response && Array.isArray(response.data)) {
                setAssociations(response.data);
            }
        } catch (error) {
            console.error("Failed to fetch associations:", error);
            toast.error("Failed to load condo associations.");
        } finally {
            setLoading(false);
        }
    };
    ...
    const handleConfirmDelete = async () => {
        if (!deletingAssociation || !deletingAssociation.id) return;
        try {
            await deleteCondoAssociationApi(deletingAssociation.id);
            toast.success("Condo association deleted successfully!");
            setDeletingAssociation(null);
            fetchAssociations();
        } catch (error) {
            console.error("Failed to delete association:", error);
            toast.error("Failed to delete condo association.");
        }
    };
    ...
    const validate = () => {
        ...
        const estStr = String(established || "").trim();
        if (!estStr) {
            tempErrors.established = "Established year is required";
            isValid = false;
        } else if (isNaN(Number(estStr)) || Number(estStr) < 1800 || Number(estStr) > new Date().getFullYear()) {
    ...
    const handleOpenEditModal = (association: CondoAssociationType, e: React.MouseEvent) => {
        ...
        setEstablished(association.established ? association.established.toString() : "");
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            if (selectedAssociation && selectedAssociation.id) {
                // Edit Mode
                await updateCondoAssociationApi(selectedAssociation.id, {
                    name,
                    address,
                    established,
                    status,
                ...
                // Add Mode
                await createCondoAssociationApi({
                    name,
                    address,
                    established,
                    status,
            ...
            toast.error("Failed to save condo association.");
        } finally {
            setIsSaving(false);
        }
    };
                ...
                <div className="overflow-x-auto">
                    {loading ? (
                        <div className="flex flex-col items-center justify-center py-16 gap-3">
                            ...
                            <span className="text-xs text-slate-400 font-semibold">Loading condo associations...</span>
                        </div>
                    ) : (
                                            ...
                                            <td className="py-4 px-6" onClick={(e) => e.stopPropagation()}>
                                                <div className="flex items-center justify-center gap-3">
                                                    ...
                                                    <button
                                                        onClick={(e) => {
                                                            e.stopPropagation();
                                                            setDeletingAssociation(assoc);
                                                        }}
                                                        className="p-1 hover:bg-slate-100 rounded transition-colors cursor-pointer"
                                                        title="Delete Condo Association"
                                                    >
                                                        <svg
                                                            xmlns="http://www.w3.org/2000/svg"
                                                            fill="none"
