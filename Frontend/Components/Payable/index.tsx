...
export default function Payable() {
    const [payables, setPayables] = useState<PayableType[]>([
        {
            id: 1,
            payTo: "Apex Plumbing Services",
            dateOfPayment: "2026-08-01",
            amount: 250.00,
            status: "Paid",
            instrument: "ACH"
        },
        {
            id: 2,
            payTo: "Greenscape Landscaping",
            dateOfPayment: "2026-08-10",
            amount: 320.00,
            status: "Pending",
            instrument: "ACH"
        },
        {
            id: 3,
            payTo: "Millbrook Pest Solutions",
            dateOfPayment: "2026-08-05",
            amount: 120.00,
            status: "Failed",
            instrument: "Cheque"
        }
    ...
    const [status, setStatus] = useState<"Paid" | "Pending" | "Failed">("Pending");
    ...
    const [deletingPayable, setDeletingPayable] = useState<PayableType | null>(null);

    const filteredPayables = payables.filter((item) => {
        const matchesSearch =
            item.payTo.toLowerCase().includes(searchTerm.toLowerCase()) ||
    ...
    const handleConfirmDelete = () => {
        ...
        setPayables(prev => prev.filter(item => item.id !== deletingPayable.id));
    ...
    const handleOpenAddModal = () => {
        ...
        setStatus("Pending");
        setInstrument("ACH");
        setErrors({ payTo: "", dateOfPayment: "", amount: "" } as any);
        setIsModalOpen(true);
    };

    const handleOpenEditModal = (item: PayableType) => {
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        if (selectedPayable) {
            ...
            setPayables(prev =>
                prev.map(item =>
                    item.id === selectedPayable.id
                        ? { ...item, payTo, dateOfPayment, amount: parseFloat(amount), status, instrument }
                        : item
                )
            );
            ...
            setPayables(prev => [...prev, newRecord]);
                            ...
                            {filteredPayables.length > 0 ? (
                                filteredPayables.map((item) => (
                                    <tr key={item.id} className="hover:bg-slate-50/50 transition-colors">
                                        <td className="py-4 px-6 font-bold text-slate-800">{item.payTo}</td>
                                        <td className="py-4 px-6 text-slate-500 font-sans">{item.dateOfPayment}</td>
                    ...
                    <div className="bg-white w-full max-w-lg rounded-2xl p-6 border border-slate-100 shadow-xl space-y-4">
                        ...
                        <form onSubmit={handleSubmit} noValidate className="space-y-4 text-xs font-semibold text-slate-700">
                            ...
                            <div className="pt-2 flex justify-end gap-3">
                                ...
                                >
                                    {selectedPayable ? "Update Payable" : "Save Payable"}
                                </button>
                            </div>
                        </form>
                    </div>
                ...
                <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-xs flex items-center justify-center z-50 p-4">
                    <div className="bg-white w-full max-w-md rounded-2xl p-6 border border-slate-100 shadow-xl space-y-4 animate-in fade-in zoom-in-95 duration-200">
                        ...
                        <p className="text-xs text-slate-500 font-semibold leading-relaxed">
                            Are you sure you want to delete payable record for <span className="font-extrabold text-slate-800">{deletingPayable.payTo}</span>? This action is permanent and cannot be undone.
                        </p>
